...
type idleConn struct {
	...
	state        atomic.Int32
	lastActivity atomic.Int64 // UnixNano 时间戳，替代每次 Reset timer
	once         sync.Once
	err          error
...
func newIdleConn(raw net.Conn, idleTimeout, gracePeriod time.Duration, forceReset bool) *idleConn {
	...
	if idleTimeout > 0 {
		// 周期性检查，间隔为 idleTimeout 的一半，最小 5s
		interval := idleTimeout / 2
		if interval < 5*time.Second {
			interval = 5 * time.Second
		}
		ic.idleTimer = time.AfterFunc(interval, ic.checkIdle)
	}
	return ic
}

// checkIdle 周期性检查是否空闲超时，替代每次 I/O 都 Reset timer
func (c *idleConn) checkIdle() {
	...
	// 还没到期，重新调度，等待剩余时间
	remaining := c.idleTimeout - elapsed
	c.mu.Lock()
	if c.idleTimer != nil && c.state.Load() == stateNormal {
